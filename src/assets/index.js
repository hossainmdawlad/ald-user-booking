...
document.querySelector('button#prev_feedback_submit').addEventListener('click',function (e) { 
    ...
    .then(json => {
        ...
        if (json.success == true) {
            // console.log(json.message);
            // console.log(json.error);
            document.querySelector('#prev_feedback_status').className = '';
            document.querySelector('#prev_feedback_status').classList.add('status','success');
            document.querySelector('#prev_feedback_status').innerHTML = json.data;
        }else{
            document.querySelector('#prev_feedback_status').className = '';
            document.querySelector('#prev_feedback_status').classList.add('status','warning');
            document.querySelector('#prev_feedback_status').innerHTML = json;
        }
    })
    .catch(err => {
        console.log('Request Failed', err);
        document.querySelector('#prev_feedback_status').className = '';
        document.querySelector('#prev_feedback_status').classList.add('status','error');
        document.querySelector('#prev_feedback_status').innerHTML = err;
    });
});
